...
```bash
hdiutil attach src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg -nobrowse
...
```bash
xcrun notarytool submit src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg \
...
```bash
xcrun stapler staple src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg
```
...
```bash
xcrun stapler validate src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg
```

Verify Gatekeeper acceptance:

```bash
spctl -a -vvv -t install src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg
```
...
```bash
codesign -dv --verbose=4 src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg
```
...
```bash
gh release upload v0.4.2 src-tauri/target/release/bundle/dmg/C3_0.4.2_aarch64.dmg --clobber
```

Verify the release asset:

```bash
gh release view v0.4.2 --json url,assets --jq '.url, (.assets[] | select(.name=="C3_0.4.2_aarch64.dmg") | {name,url,size: .size})'
```
